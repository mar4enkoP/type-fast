...
export class TabsComponent {
    ...
    private readonly splits: Array<{ lines: HTMLElement[], destroy(): void }> = [];
    ...
    private animate(nextIndex: number): void {
        const currentLines = this.splits[this.activeIndex].lines;
        const nextLines = this.splits[nextIndex].lines;
        currentLines.forEach((line, i) => {
            line.animate(
                [{transform: 'translateY(0%)'}, {transform: 'translateY(150%)'},],
                {duration: DURATION_MS, delay: i * STAGGER_MS, fill: 'forwards', easing: 'ease-in'}
            );
        })
